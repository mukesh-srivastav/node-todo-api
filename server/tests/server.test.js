...
describe('DELETE /todos/:id', () => {
	it('should remove a todo doc', (done) => {
		request(app)
			...
			.end((err, res) => {
				...
				// query db using findById not not exist.
	...
	it('should not remove a todo doc of others', (done) => {
		request(app)
			.delete(`/todos/${todos[0]._id.toHexString()}`)
			.set('x-auth', users[1].tokens[0].token)
			.expect(404)
...
describe('POST /users', () => {
	...
	it('should return validatior errors if request invalid', (done) => {
		request(app)
			.post('/users')
			.send({
				email: 'and',
